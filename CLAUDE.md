...
**Audit de sécurité RGPD/CNIL du 2026-08-24** (3 agents en parallèle : infra Supabase déployée, code applicatif, conformité RGPD). Infra : nettoyage du 23 août confirmé complet (6 Edge Functions attendues et pas une de plus, RLS propre sur les 7 tables, `uri_allow_list` sans trace de Vercel, `billops_sync`/`login_relay`/`oauth_pending` bien supprimées, pas juste vidées) ; secrets obsolètes de l'ancien flow Gmail-send (`GOOGLE_CLIENT_ID`/`_SECRET`, `SENDGRID_API_KEY`) supprimés, `EXECUTE` sur `handle_new_user()` révoqué pour `anon`/`authenticated` (advisor WARN, risque réel faible). Protection mot de passe fuité (HaveIBeenPwned) indisponible sur le plan Supabase actuel (réservée au plan Pro) — à activer si upgrade. Code : le pattern d'échappement `esc()` (introduit lors d'un fix XSS antérieur) n'avait pas été appliqué partout — `showToast()` (sink `innerHTML` central, ~50 appelants) recevait du texte interpolé non échappé (`t()` ne échappe jamais ses variables), atteignable via un simple message d'erreur de validation (email client invalide) et exécuté dans le renderer privilégié ; corrigé en passant `showToast` sur `textContent`. Deux XSS stockées supplémentaires trouvées et fixées : la description d'une ligne de facture (`addItem`, injectée dans un `<textarea>` sans échappement, cassable via `</textarea><img onerror=...>`) et le nom de client dans l'onglet Chiffre d'affaires. `send-invoice` vérifiait bien un token de session Supabase mais jamais l'abonnement actif : un compte inscrit gratuitement (sans jamais payer) pouvait l'utiliser comme relais email ouvert sur le domaine authentifié `mail.ops-suite.fr` — fix : même vérification `subscriptions.expires_at` que `check-access`. CSP ajoutée sur `facture.html`/`mobile/index.html` (absente jusqu'ici) en défense en profondeur. `supabase/README.md` était resté figé sur l'archi d'avant le 23 août (documentait encore le déploiement des fonctions/relais OAuth supprimés) — réécrit à jour, pour éviter qu'un futur redéploiement suivant ce README ne recrée la faille déjà fermée. RGPD : gaps identifiés côté code (mobile sans aucune mention légale/RGPD alors qu'il traite les mêmes données — modal portée depuis `facture.html` ; lien vers les mentions légales absent des écrans `#loginGate`/`#paywallGate`, ajouté ; Stripe absent du texte RGPD malgré le traitement de données de paiement, ajouté) ; gaps hors code identifiés mais **non traités par cet audit** (CGV, registre des traitements Art. 30, procédure de notification de violation 72h Art. 33-34 — y compris une évaluation rétroactive à faire pour les incidents du 23 août, DPA à obtenir auprès de Supabase/Brevo/Stripe, politique de durée de conservation formelle) : ce sont des décisions/documents qui reviennent à l'éditeur, pas des corrections de code.

Fichiers : `facture.html` (app desktop complète, single-file, packagée en app Mac via Electron), `mobile/index.html` (PWA compagnon iPhone, single-file, déployée sur GitHub Pages), `supabase/functions/` (Edge Functions : `send-invoice` envoi centralisé via l'API Brevo depuis `mail.ops-suite.fr`, protégé par un vrai token de session Supabase depuis le 2026-08-23 et un abonnement actif depuis le 2026-08-24 (en plus du secret partagé) ; `check-access`/`stripe-checkout`/`stripe-webhook` login + abonnement ; `delete-account` suppression de compte ; `notify-upcoming-bookings` rappels push). `send-invoice-server.js` (ancienne version Express, référence obsolète pré-SMTP, pas utilisée en prod).
